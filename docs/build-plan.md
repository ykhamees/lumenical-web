...
| A-4 (2026-08-22, revised) | **The admin console is a separate app (`admin/`), server-rendered, deployed independently to Cloud Run and served at `lumenical.com/website/**` via a Hosting rewrite** — not its own subdomain; a same-day revision once `web-admin.lumenical.com` turned out to need to coexist with another product's `/api/*`-style routing already live in this GCP org. Routes (`/leads/`, `/pages/`, …, no `/admin` prefix — the whole app is admin now, under `basePath: "/website"`) with client-side data fetching, unchanged from before. It reaches the Python API through its own backend-for-frontend proxy (`admin/src/app/api/[...path]/route.ts`, served publicly at `/website/api/**`), not a Firebase Hosting rewrite. | The owner chose to segregate deployments — admin's operational surface (auth, GCP IAM, incident blast radius) is now fully independent of the marketing site's static export. Going server-rendered isn't incidental: `admin/src/proxy.ts` verifies a session cookie before any admin page renders, replacing the old "client-side check is UX only" model with real server-side gating. `firestore.rules` and the Python API's own bearer-token check (`require_admin_user`/`require_admin_only`) remain the authoritative authorization layer either way — though the *header* the Firebase token travels in changed too: the Cloud-Run-to-Cloud-Run identity token needs the standard `Authorization` header (confirmed against live infra), so the Firebase user token moved to a custom `X-Firebase-Id-Token` header instead. Cost: two design-token files (`tailwind.config.ts`, `globals.css`) to keep in sync by hand, and one cross-app contract (`/api/admin/**` on the Python API side) to coordinate when it changes — accepted in exchange for deployment independence. See `docs/infrastructure-admin.md`. |
...
**Admin** — a separate app (`admin/`), deployed independently to Cloud Run
and served at `lumenical.com/website/**` via a Hosting rewrite, not its own
subdomain (A-4, superseded 2026-08-22, revised again the same day once a
subdomain turned out to collide with another product's routing in this
GCP org); every route `noindex` via the app's root layout, plus a blanket
`robots.ts`
