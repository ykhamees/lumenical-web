...
- Hosting: static export on Firebase Hosting (project `lumenical-ai`), custom domain `lumenical.com`, `/api/**` rewritten to Cloud Run per `firebase.json` once that service exists — both keep standard server logs outside this site's own code.
