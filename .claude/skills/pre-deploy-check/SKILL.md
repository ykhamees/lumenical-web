...
- Every top-level `src/app/**/page.tsx` route segment has a matching entry in `src/content/routes.ts` (the single source of truth `navLinks`/`footerLinks` derive from) — a page with no registry entry, or a registry entry with no page, is drift. The `/services/[slug]/`, `/platforms/[slug]/`, `/demos/[slug]/`, and `/insights/[slug]/` detail routes aren't in `routes.ts`; check instead that `sitemap.ts` enumerates them (statically for services/platforms, dynamically from Firestore for demos/insights) and that `services.ts`/`platforms.ts` still have exactly six/three entries. **Deliberate exception:** `/demos/page.tsx` and `/insights/page.tsx` exist unconditionally but only get a `routes.ts` entry when `NEXT_PUBLIC_CMS_LIVE=true` (`docs/build-plan.md` 5.1) — flag as drift only if a `routes.ts` entry exists pointing at a page that doesn't, not the reverse for these two.
